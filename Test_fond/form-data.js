...
// ============================================
// ÎLES LOYAUTÃ‰ - Communes et Districts coutumiers
// ============================================
const communesIles = {
    "Ouvéa": [
        "Fayaoué",
...
const collegesPublics = [
    ...
    "Nouméa - Collège FranÃ§ois Ollivaud (Portes de Fer) et SEG",
...
const lyceesPublics = [
    ...
    "Nouméa - Lycée professionnel commercial et hÃ´telier A. Escoffier",
...
// Lycées privés
const lyceesPrives = [
    "Bourail - Lycée professionnel FranÃ§ois d'Assise (DDEC)",
...
// ============================================
// FONCTIONS POUR LES MENUS DÃ‰ROULANTS
// ============================================

// Ecoles primaires (charges depuis register/primary-schools-data.js)
const primarySchoolsData = window.primarySchoolsByProvinceAndCommune || {};
const primarySchoolProvincePreferredOrder = ['Province Sud', 'Province Nord', 'Îles Loyauté', 'Province des Îles Loyauté'];

function loadPrimaryProvinces() {
    const selectProvincePrimaire = document.getElementById('primaire-province-select');
    if (!selectProvincePrimaire) return;

    selectProvincePrimaire.innerHTML = '<option value="">Province de l\'école</option>';
    const dataProvinces = Object.keys(primarySchoolsData);
    const orderedProvinces = [
        ...primarySchoolProvincePreferredOrder.filter(province => dataProvinces.includes(province)),
        ...dataProvinces
            .filter(province => !primarySchoolProvincePreferredOrder.includes(province))
            .sort((a, b) => a.localeCompare(b, 'fr'))
    ];

    orderedProvinces.forEach(province => {
        const option = document.createElement('option');
        option.value = province;
        option.textContent = province;
        selectProvincePrimaire.appendChild(option);
    });
}

function resetPrimaryCommuneSelect() {
    const selectCommunePrimaire = document.getElementById('primaire-commune-select');
    if (!selectCommunePrimaire) return;

    selectCommunePrimaire.innerHTML = '<option value="">Commune de l\'école</option>';
    selectCommunePrimaire.value = '';
    selectCommunePrimaire.disabled = true;
}

function resetPrimarySchoolSelect() {
    const selectEtablissement = document.getElementById('etablissement-select');
    if (!selectEtablissement) return;

    selectEtablissement.innerHTML = '<option value="">Sélectionnez d\'abord une commune</option>';
    selectEtablissement.value = '';
    selectEtablissement.disabled = true;
}

function loadPrimaryCommunes(province) {
    const selectCommunePrimaire = document.getElementById('primaire-commune-select');
    if (!selectCommunePrimaire) return;

    resetPrimaryCommuneSelect();
    if (!province || !primarySchoolsData[province]) {
        resetPrimarySchoolSelect();
        return;
    }

    const communes = Object.keys(primarySchoolsData[province]).sort((a, b) => a.localeCompare(b, 'fr'));
    communes.forEach(commune => {
        const option = document.createElement('option');
        option.value = commune;
        option.textContent = commune;
        selectCommunePrimaire.appendChild(option);
    });
    selectCommunePrimaire.disabled = false;
    resetPrimarySchoolSelect();
}

function loadPrimarySchools(province, commune) {
    const selectEtablissement = document.getElementById('etablissement-select');
    if (!selectEtablissement) return;

    selectEtablissement.innerHTML = '<option value="">Sélectionnez une école primaire</option>';
    selectEtablissement.value = '';
    selectEtablissement.disabled = true;

    if (!province || !commune || !primarySchoolsData[province] || !primarySchoolsData[province][commune]) {
        selectEtablissement.innerHTML = '<option value="">Sélectionnez d\'abord une commune</option>';
        return;
    }

    const schools = primarySchoolsData[province][commune];
    if (!Array.isArray(schools) || schools.length === 0) {
        selectEtablissement.innerHTML = '<option value="">Aucune école trouvée</option>';
        return;
    }
...
function loadEtablissements(type) {
    ...
    const primaireResidenceFields = document.getElementById('primaire-residence-fields');
    const selectProvincePrimaire = document.getElementById('primaire-province-select');
    ...
    if (primaireResidenceFields) {
        primaireResidenceFields.style.display = 'none';
    }
    if (selectProvincePrimaire) {
        selectProvincePrimaire.value = '';
    }
    resetPrimaryCommuneSelect();
    ...
    if (type === 'École primaire') {
        if (primaireResidenceFields) {
            primaireResidenceFields.style.display = 'block';
        }
        loadPrimaryProvinces();
        resetPrimaryCommuneSelect();
        resetPrimarySchoolSelect();
        return;
    }
    ...
    if (type === 'College') {
        ...
        etablissements.sort((a, b) => a.localeCompare(b, 'fr'));
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Ã‰couter les changements sur le select province
    const selectProvince = document.getElementById('province-select');
    if (selectProvince) {
    ...
    // Ã‰couter les changements sur le select commune
    const selectCommune = document.getElementById('commune-select');
    if (selectCommune) {
    ...
    // Ecouter les changements pour le filtre primaire (independant de l'adresse personnelle)
    const selectProvincePrimaire = document.getElementById('primaire-province-select');
    if (selectProvincePrimaire) {
        selectProvincePrimaire.addEventListener('change', function() {
            loadPrimaryCommunes(this.value);
        });
    }

    const selectCommunePrimaire = document.getElementById('primaire-commune-select');
    if (selectCommunePrimaire) {
        selectCommunePrimaire.addEventListener('change', function() {
            const provincePrimaire = document.getElementById('primaire-province-select')?.value || '';
            loadPrimarySchools(provincePrimaire, this.value);
        });
    }
    
    // Ã‰couter les changements sur le select type d'établissement
