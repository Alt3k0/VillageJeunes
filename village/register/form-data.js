...
function loadEcoleProvinces() {
    const sel = document.getElementById("ecole-province-select");
    if (!sel) return;

    sel.innerHTML = '<option value="">Sélectionnez une province</option>';
    Object.keys(ecolesPrimaires)
        .sort()
        .forEach((p) => {
...
function loadEcoleCommunes(province) {
    const selCommune = document.getElementById("ecole-commune-select");
    const selEcole = document.getElementById("ecole-primaire-select");
    if (!selCommune || !selEcole) return;

    // reset écoles
    selEcole.innerHTML = '<option value="">Sélectionnez une école</option>';
    selEcole.disabled = true;

    selCommune.innerHTML = '<option value="">Sélectionnez une commune</option>';
    selCommune.disabled = true;

    if (!province || !ecolesPrimaires[province]) return;

    const communes = Object.keys(ecolesPrimaires[province]).sort((a, b) =>
...
function loadEcolesPrimaires(province, commune) {
    const selEcole = document.getElementById("ecole-primaire-select");
    if (!selEcole) return;
    ...
    const list = ecolesPrimaires?.[province]?.[commune];
    ...
    list.forEach((nom) => {
        ...
        opt.value = nom;
        opt.textContent = nom;
        selEcole.appendChild(opt);
    });

    selEcole.disabled = false;
}
